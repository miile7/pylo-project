...
class Controller:
    ...
    def __init__(self, view: typing.Optional[AbstractView]=None,
        ...
        self._logger = logging.Logger("pylo.Controller")
        if self._logger.isEnabledFor(logging.DEBUG):
            self._log_debug = True
        else:
            self._log_debug = False
        if self._log_debug:
            self._logger.debug("Creating new instance of controller")
        ...
        while ((not isinstance(self.microscope, MicroscopeInterface) or
            ...
            for name, kind in zip(names, load_kinds):
                ...
                try:
                    ...
                    self._logger.error("{}: {}".format(e.__class__.__name__, e),
                                       exc_info=e)
            ...
            while (not isinstance(self.measurement, Measurement) and 
                ...
                if (not isinstance(interactions, typing.Sequence) or 
                    ...
                    err = RuntimeError("The view does not return all required " + 
                                       "user values.")
                    self._logger.error("{}: {}".format(err.__class__.__name__,
                                       err), exc_info=err)
                    raise err
